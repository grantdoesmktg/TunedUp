...
export const PLAN_TOKENS = {
  ...
  PRO: 250,
  ULTRA: 500,
  ADMIN: 999999  // PostgreSQL can't store Infinity, use large integer
}
...
export async function hasEnoughTokens(email, toolType, fingerprint) {
  ...
  try {
    ...
    // Admin gets unlimited tokens (display as large number for UX)
    if (user.planCode === 'ADMIN') {
      return {
        allowed: true,
        tokens: 999999,  // Stored as large integer, effectively unlimited
        cost
      }
    }
...
export function calculateMonthlyRefill(planCode, currentTokens) {
  ...
  // Admin tokens are already 999999, no special handling needed
  // (kept for backward compatibility if planAllocation is ever Infinity)
  if (planAllocation === Infinity || planAllocation === 999999) {
    return 999999
  }

  // No carryover - just return plan allocation
  if (!carryoverRule.enabled) {
