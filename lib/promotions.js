...
export async function checkPromotion(code) {
  try {
    console.log('[checkPromotion] Starting check for code:', code)

    // Try to create FIRST50 promotion if it doesn't exist
    if (code === 'FIRST50') {
      try {
        await createFirst50Promotion()
      } catch (createError) {
        console.log('[checkPromotion] Could not auto-create FIRST50:', createError.message)
      }
    }
    ...
    console.log('[checkPromotion] Found promotion:', promotion ? promotion.code : 'null')
    ...
    console.error('[checkPromotion] Error name:', error.name)
    console.error('[checkPromotion] Error code:', error.code)
    console.error('[checkPromotion] Error message:', error.message)
    console.error('[checkPromotion] Error stack:', error.stack)
