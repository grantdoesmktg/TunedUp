...
const API_URL = 'https://www.tunedup.dev/api/turbo-tycoon';
...
export default function TurboTycoonScreen() {
  ...
  const fetchAPI = async (action: string, params: any = {}) => {
    try {
      console.log(`🌐 Turbo API Request [${action}]:`, API_URL);
      ...
      console.log(`📤 Request payload [${action}]:`, JSON.stringify(payload, null, 2));
      ...
      console.log(`📡 Turbo API Response status [${action}]:`, response.status);

      if (!response.ok) {
        const errorText = await response.text();
        console.error(`❌ Response not OK [${action}]:`, response.status, errorText);
        return { success: false, error: `Server error: ${response.status}` };
      }

      const data = await response.json();
      console.log(`✅ Turbo API Response data [${action}]:`, JSON.stringify(data, null, 2));
      return data;
    } catch (error) {
      console.error(`❌ Turbo API Error [${action}]:`, error);
      console.error(`❌ Error details:`, JSON.stringify(error, Object.getOwnPropertyNames(error)));
      Alert.alert("Network Error", `Failed to ${action}: ${error}`);
  ...
  const parseServerState = (serverState: any): GameState => {
    // Backend returns 'hp' but frontend uses 'totalHp'
    const totalHp = serverState.totalHp || serverState.hp || '0';
    const lifetimeHp = serverState.lifetimeHpEarned || serverState.lifetimeHp || '0';

    // Backend returns parts as array [{partId, level}], convert to object {partId: level}
    let partsObj: Record<string, number> = {};
    if (Array.isArray(serverState.parts)) {
      serverState.parts.forEach((part: any) => {
        partsObj[part.partId] = part.level;
      });
    } else if (serverState.parts) {
      partsObj = serverState.parts;
    }

    return {
      totalHp: BigInt(totalHp),
      lifetimeHpEarned: BigInt(lifetimeHp),
      currentTier: serverState.currentTier,
      currentRpm: serverState.currentRpm,
      currentGear: serverState.currentGear,
      lastClickTime: serverState.lastClickTime,
      redzoneStartTime: serverState.redzoneStartTime,
      throttleLevel: serverState.throttleLevel,
      ecuLevel: serverState.ecuLevel,
      parts: partsObj,
      tokens: serverState.tokens,
      tokensEarnedToday: serverState.tokensEarnedToday,
      lastTokenDate: serverState.lastTokenDate,
  ...
  useEffect(() => {
    ...
    // Sync Interval (5s) - DISABLED: Production backend doesn't have sync endpoint yet
  ...
  const handleBuyPart = async (partId: string) => {
     console.log('🔧 handleBuyPart called with partId:', partId);
     console.log('🔧 Current state before purchase:', {
        totalHp: gameState.totalHp.toString(),
        parts: gameState.parts
     });
     const res = await fetchAPI('buy_part', { partId });
     console.log('🔧 buy_part API response:', res);
     if (res.success && res.state) {
        const newState = parseServerState(res.state);
        console.log('🔧 Parsed new state:', {
           totalHp: newState.totalHp.toString(),
           parts: newState.parts
        });
        setGameState(newState);
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
     } else {
        console.error('🔧 Purchase failed:', res.error);
        Alert.alert("Purchase Failed", res.error || "Not enough HP");
     }
  };

  const handleBuyManual = async (type: 'throttle' | 'ecu') => {
     console.log('🔧 handleBuyManual called with type:', type);
     console.log('🔧 Current levels before upgrade:', {
        throttleLevel: gameState.throttleLevel,
        ecuLevel: gameState.ecuLevel,
        totalHp: gameState.totalHp.toString()
     });
     const res = await fetchAPI('buy_manual_upgrade', { upgradeType: type });
     console.log('🔧 buy_manual_upgrade API response:', res);
     if (res.success && res.state) {
        const newState = parseServerState(res.state);
        console.log('🔧 Parsed new state:', {
           throttleLevel: newState.throttleLevel,
           ecuLevel: newState.ecuLevel,
           totalHp: newState.totalHp.toString()
        });
        setGameState(newState);
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
     } else {
        console.error('🔧 Manual upgrade failed:', res.error);
        Alert.alert("Upgrade Failed", res.error || "Not enough HP");
     }
  };
